    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          <select value={provider} onChange={(e)=>setProvider(e.target.value)} className="rounded-lg border border-gray-300 bg-white px-3 py-2">
            <option value="kolis">KOLIS-NET</option>
            <option value="seoji">서지(ISBN)</option>
          </select>
          <input value={q} onChange={(e)=>setQ(e.target.value)} onKeyDown={(e)=> e.key==="Enter" && search()} placeholder="도서명/저자 등 입력 후 Enter" className="flex-1 rounded-lg border border-gray-300 bg-white px-3 py-2" />
        ...
        {loading ? Array.from({length:6}).map((_,i)=>(
          ...
          <div key={idx} className="flex gap-3 rounded-lg border border-gray-200 p-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-gray-600 mt-0.5 line-clamp-1">{it.author}</div>
              <div className="text-[11px] text-gray-400 line-clamp-1">{it.publisher} {it.ISBN ? `· ${it.ISBN}` : ""}</div>
              <div className="mt-2">
                <button type="button" onClick={()=>onApply(it)} className="rounded bg-blue-50 px-2 py-1 text-xs text-blue-600 hover:bg-blue-100">이 항목 적용</button>
              </div>
            </div>
          </div>
        ))}
...
export default function BookForm() {
  ...
  const applyFromKorlib = (it)=>{
    ...
    setIsbn(it.ISBN || it.isbn || "");
    setImage(it.image || "");
    if(!description && it.description) setDescription(it.description);
  };
  ...
  const handleSubmit = async (e)=>{
    ...
    const payload = {
      ...
      registrant,
      email, "e-mail": email,
      title, author, translator, publisher, isbn, theme, level, division,
          ...
          <section className="lg:col-span-4">
            <form onSubmit={handleSubmit} className="rounded-2xl border border-gray-200 bg-white p-6 shadow">
              ...
              <div className="mt-8 flex items-center justify-end gap-3">
                <button type="button" onClick={()=>history.back()} className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 hover:bg-gray-100">취소</button>
                <button type="submit" disabled={submitting || !requiredOk} className={`rounded-lg px-4 py-2 text-white ${submitting || !requiredOk ? "bg-blue-300" : "bg-blue-600 hover:bg-blue-700"}`}>
